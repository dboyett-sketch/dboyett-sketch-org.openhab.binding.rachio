...
public class RachioHttp {
    ...
    /**
     * ===== Rate limiting header handling with callback support =====
    ...
    private static void handleRateLimitHeaders(HttpResponse<String> response) {
        ...
        if (remainingOpt.isPresent()) {
            try {
                ...
                int percentRemaining = (limit > 0) ? (int) ((remaining / (double) limit) * 100) : 100;
        ...
        // ===== Notify bridge handler of rate limit data =====
        if (remaining >= 0) { // Only notify if we successfully parsed remaining
            RateLimitCallback callback = rateLimitCallback;
            if (callback != null) {
                try {
