...
public class RachioZoneHandler extends RachioHandler {
    ...
    private @Nullable ZoneRunStatus lastRunStatus;

    // ===== CRITICAL PATCH: Flag to track if channels have been created =====
    ...
    public void initialize() {
        ...
        // ===== CRITICAL PATCH: Create zone channels BEFORE registering with bridge =====
        createZoneChannels();

        // ===== PATCHED (Line 60): Fixed getBridgeHandler() call - use getBridge().getHandler() =====
        ThingHandler bridgeHandler = getBridge() != null ? getBridge().getHandler() : null;
    ...
    // ===== PATCHED: Added missing abstract method implementation =====
    @Override
        ...
        if (zoneData != null) {
            switch (channelId) {
                ...
                default:
                    ...
                    refreshZone(); // Fall back to full zone refresh
            }
        } else {
            refreshZone(); // No data yet, get fresh data
        }
    }

    /**
     * ===== CRITICAL PATCH: Create all zone channels =====
     * Implements OpenHAB 5.x ChannelBuilder pattern for dynamic channel creation.
     * This follows README.txt requirement: "Professional Data Exposure" for soil, crop, nozzle, efficiency monitoring
     * And aligns with Rachio API documentation for professional irrigation data.
     */
    private void createZoneChannels() {
        ...
        try {
            ...
            // Using ACTUAL fields from RachioZone.java, not the missing ones from original code
    ...
    public void handleCommand(ChannelUID channelUID, Command command) {
        ...
        if (command instanceof RefreshType) {
            // Use the base class method which calls performRefresh
        ...
        // ===== PATCHED (Line 488): Fixed getBridgeHandler() call =====
        ThingHandler bridgeHandler = getBridge() != null ? getBridge().getHandler() : null;
        if (!(bridgeHandler instanceof RachioBridgeHandler)) {
            ...
            switch (channelId) {
                case RachioBindingConstants.CHANNEL_ZONE_RUN:
                    if (command instanceof OnOffType) {
                        // ===== PATCHED (Line 503): Fixed startZone call - added duration =====
                ...
                case RachioBindingConstants.CHANNEL_ZONE_ENABLED:
                    if (command instanceof OnOffType) {
                        // ===== PATCHED: Correct method exists in bridge handler =====
                ...
                case RachioBindingConstants.CHANNEL_ZONE_RUNTIME:
                    if (command instanceof DecimalType) {
                        ...
                        // ===== PATCHED (Line 517): Use correct field update instead of nonexistent method =====
                        // We'll update the local zone data and let it sync on next poll
                ...
                case RachioBindingConstants.CHANNEL_ZONE_DURATION:
                    if (command instanceof DecimalType) {
                        ...
                        // ===== PATCHED (Line 523): Use setZoneEnabled for duration changes =====
                        // Duration changes might require different handling - for now log it
    ...
    private void refreshZone() {
        // ===== PATCHED (Line 538): Fixed getBridgeHandler() call =====
        ThingHandler bridgeHandler = getBridge() != null ? getBridge().getHandler() : null;
        if (bridgeHandler instanceof RachioBridgeHandler) {
            RachioZoneConfiguration localConfig = config;
            if (localConfig != null) {
                // ===== PATCHED (Line 540): Use getZoneData instead of nonexistent refreshZone =====
     ...
     * PATCHED: Uses ACTUAL fields from RachioZone.java, not the missing ones
     */
    public void updateZoneData(@Nullable RachioZone zoneData) {
        ...
        // ===== PATCHED: Update using ACTUAL fields from RachioZone.java =====

        // Update professional irrigation data if available
        if (zoneData.soil != null) {
        ...
        // ===== PATCHED: Use actual efficiency field (double, not Double) =====
        // ===== PATCHED (Line 578): Fixed double != null comparison =====
        // efficiency is primitive double, so no null check needed
        updateState(RachioBindingConstants.CHANNEL_ZONE_IRRIGATION_EFFICIENCY, new DecimalType(zoneData.efficiency));

        // ===== PATCHED: Update water management channels with ACTUAL fields =====

        // Water Available - using availableWater field
        // ===== PATCHED (Line 588): Use MetricPrefix.MILLI(SIUnits.METRE) instead of Units.MILLIMETRE =====
        ...
        // ===== PATCHED (Line 593): Use MetricPrefix.MILLI(SIUnits.METRE) instead of Units.MILLIMETRE =====
        ...
        // ===== PATCHED (Line 598): Use MetricPrefix.MILLI(SIUnits.METRE) instead of Units.MILLIMETRE =====
        ...
        // ===== PATCHED (Line 603-608): Use managementAllowedDepletion instead of moisture/depletion =====
        ...
        // ===== PATCHED (Line 608): Use SIUnits.SQUARE_METRE instead of Units.SQUARE_METRE =====
     ...
     * PATCHED: Uses actual ZoneRunStatus enum methods
     */
    public void updateZoneRunStatus(@Nullable ZoneRunStatus runStatus) {
        ...
        // ===== PATCHED (Line 630): Use enum methods, not fields =====
        ...
        // ===== PATCHED (Line 633-634): Duration from zone data, not runStatus =====
    ...
    // ===== RachioStatusListener Interface Implementation (inherited from RachioHandler) =====
    ...
    // ===== PATCHED: Fixed bridge status change handler - REMOVED @Override annotation =====
    /**
     * Handle bridge status changes
     * PATCHED (Line 729): Removed @Override annotation to match base class method
     */
    ...
    public void dispose() {
        ...
        // ===== PATCHED (Line 729): Fixed getBridgeHandler() call =====
        ThingHandler bridgeHandler = getBridge() != null ? getBridge().getHandler() : null;
        if (bridgeHandler instanceof RachioBridgeHandler) {
