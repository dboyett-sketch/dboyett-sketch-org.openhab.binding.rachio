 ...
 * handlers.
 *
 * UPDATED FOR 5.0.1+: Modified to pass the RachioApiClient to Device and Zone handlers
 * via constructor injection, resolving OSGi dependency race conditions.
 *
...
public class RachioHandlerFactory extends BaseThingHandlerFactory {
    ...
    /**
     * Create device handler with dependency injection
     * UPDATED FOR 5.0.1+: Now passes rachioApiClient to constructor
     */
    private RachioDeviceHandler createDeviceHandler(Thing thing) {
        ...
        try {
            // UPDATED: Pass the injected rachioApiClient to the handler constructor
            RachioDeviceHandler handler = new RachioDeviceHandler(thing, rachioApiClient);
            logger.debug("Successfully created RachioDeviceHandler for {} with API client", thing.getUID());
            return handler;
        } catch (Exception e) {
    ...
    /**
     * Create zone handler with dependency injection
     * UPDATED FOR 5.0.1+: Now passes rachioApiClient to constructor
     */
    private RachioZoneHandler createZoneHandler(Thing thing) {
        ...
        try {
            // UPDATED: Pass the injected rachioApiClient to the handler constructor
            RachioZoneHandler handler = new RachioZoneHandler(thing, rachioApiClient);
            logger.debug("Successfully created RachioZoneHandler for {} with API client", thing.getUID());
            return handler;
        } catch (Exception e) {
