 ...
 */
@Component(service = { org.openhab.core.thing.binding.BridgeHandler.class,
        RachioBridgeHandler.class }, configurationPid = "bridge.rachio")
@NonNullByDefault
public class RachioBridgeHandler extends BaseBridgeHandler implements RachioHttp.RateLimitCallback {
    ...
    // OSGi Services injected via @Reference
    ...
    @Activate
    public RachioBridgeHandler(final Bridge bridge, @Reference HttpClientFactory httpClientFactory,
            @Reference ScheduledExecutorService scheduler) {
        super(bridge);
        this.httpClientFactory = httpClientFactory;
        this.scheduler = scheduler;
        logger.debug("RachioBridgeHandler activated with corrected service injection");
    }

    // ===== OPTIONAL SERVICE SETTERS =====
    ...
    public void initialize() {
        ...
        String apiKeyValue = config.apiKey;
        ...
        // ===== CRITICAL PATCH: Create rate limit channels BEFORE initializing API client =====
        createRateLimitChannels();

        this.apiClient = new RachioApiClient();
        this.apiClient.initialize(apiKeyValue);
        logger.debug("API client initialized with provided key");

        // ===== CRITICAL FIX: Register rate limit callback =====
    ...
    /**
     * ===== CRITICAL PATCH: Create all rate limit dashboard channels =====
     * Implements OpenHAB 5.x ChannelBuilder pattern for dynamic channel creation.
     * This follows README.txt requirement: "4 real-time channels for rate limit status"
     * And aligns with Rachio API documentation for rate limit headers.
     */
    ...
    public void dispose() {
        ...
        // ===== CRITICAL FIX: Unregister rate limit callback =====
    ...
    public void onRateLimitUpdate(int remaining, int limit, long reset, String status) {
        ...
        // Update bridge state on the scheduler thread for thread safety
        scheduler.submit(() -> {
     ...
     * Implements the logic from README.txt:
    ...
    private void processDevice(RachioDevice device) {
        ...
        // PATCHED (Line 227-228): Handle null device ID safely
        if (deviceId == null) {
    ...
    public void setZoneEnabled(String zoneId, boolean enabled) {
        ...
        // Note: Rachio API may not have direct zone enable/disable endpoint
        // This might need to be implemented via device configuration update
        ...
        if (zone != null) {
            ...
            // PATCHED (Line 327): Handle null deviceId safely
            String deviceId = zone.deviceId;
            notifyZoneUpdated(deviceId, zone);
